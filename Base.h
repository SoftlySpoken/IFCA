...
public:
    ...
    Base(string dPath , string uPath, string qPath , int qSize, double theta_, double alpha_, int num_partitions): Graph(dPath, uPath, qPath, qSize)
    {
    ...
    int PushContractTraversal(int sourceNode, int targetNode, double init_rmax)
    {
        if( sourceNode == targetNode ) return 0;
        if( outDeg[sourceNode] == 0 || inDeg[targetNode] == 0 ) return -1;
    ...
    bool reachfinal(int sourceNode, int targetNode, int mode, double init_rmax)
    {
        ...
        switch (mode)
        {
            ...
            case 4: res = BwPush(sourceNode, targetNode, init_rmax); break;
            case 5: res = BwPushContract(sourceNode, targetNode, init_rmax); break;
            case 6: res = BwPushContractTraversal(sourceNode, targetNode, init_rmax); break;
        }
        clock_gettime(CLOCK_MONOTONIC, &end_at);
        timeQueries += timeDiff(start_at, end_at);
